...
public class Pulverizer {
    ...
    public static void addRecipe(IItemStack[] outputs, IIngredient input, int[] outChances, int durationTicks, int euPerTick) {
        if (outputs.length < 1) {
            ...
            MineTweakerAPI.apply(new AddMultipleRecipeAction<GT_Recipe>("Adding Pulverizer recipe for " + input, input, outputs, outChances, durationTicks, euPerTick) {
                @Override
                protected GT_Recipe applySingleRecipe(ArgIterator i) {
                    return RA.addPulveriserRecipeRemovable(i.nextItem(), i.nextItemArr(), i.nextIntArr(), i.nextInt(), i.nextInt());
                }

                @Override
                protected void undoSingleRecipe(GT_Recipe recipe) {
                    RA.removePulveriserRecipe(recipe);
                }
            });
        }
    }
    ...
    public static void addRecipe(IItemStack[] inputArray , ILiquidStack[] inputFluidArray , IIngredient[] outputArray, ILiquidStack[] outputFluidArray, int[] chances, int durationTicks, int euPerTick) {
        if ((inputArray.length == 0 && inputFluidArray.length == 0) || (outputArray.length == 0 && outputFluidArray.length == 0)) {
            ...
            MineTweakerAPI.apply(new AddGTDirectRecipeAction(GT_Recipe.GT_Recipe_Map.sMaceratorRecipes, "Adding Pulverizer recipe for " + Arrays.toString(outputArray) + " : " + Arrays.toString(outputFluidArray),
