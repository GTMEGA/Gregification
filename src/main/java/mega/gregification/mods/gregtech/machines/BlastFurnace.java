...
public class BlastFurnace {
    ...
    public static void addRecipe(IItemStack[] output, ILiquidStack fluidOutput, IIngredient[] input, ILiquidStack fluidInput, int durationTicks, int euPerTick, int heat) {
        if (output.length == 0 || input.length == 0) {
            ...
            MineTweakerAPI.apply(new AddMultipleRecipeAction<GT_Recipe>("Adding Blast furnace recipe for " + output[0], input[0], ArrayHelper.itemOrNull(input, 1), fluidInput, fluidOutput, output[0], ArrayHelper.itemOrNull(output, 1), durationTicks, euPerTick, heat) {
                @Override
                protected GT_Recipe applySingleRecipe(ArgIterator i) {
                    return RA.addBlastRecipeRemovable(i.nextItem(), i.nextItem(), i.nextFluid(), i.nextFluid(), i.nextItem(), i.nextItem(), i.nextInt(), i.nextInt(), i.nextInt());
                }

                @Override
                protected void undoSingleRecipe(GT_Recipe recipe) {
                    RA.removeBlastRecipe(recipe);
                }
            });
        }
    }
    ...
    public static void addRecipe(IItemStack[] inputArray ,ILiquidStack[] inputFluidArray , IIngredient[] outputArray, ILiquidStack[] outputFluidArray,int[] chances, int durationTicks, int euPerTick, int heat) {
        if ((inputArray.length == 0 && inputFluidArray.length == 0) || (outputArray.length == 0 && outputFluidArray.length == 0)) {
            ...
            MineTweakerAPI.apply(new AddGTDirectRecipeAction(GT_Recipe.GT_Recipe_Map.sBlastRecipes, "Adding Blast furnace recipe for " + Arrays.toString(outputArray) + " : " + Arrays.toString(outputFluidArray),
                ...
                protected GT_Recipe create(ArgIterator i) {
                    return new GT_Recipe(false,i.nextItemArr(),i.nextItemArr(),null,i.nextIntArr(),i.nextFluidArr(),i.nextFluidArr(),i.nextInt(),i.nextInt(),i.nextInt());
                }
            });
        }
    }
