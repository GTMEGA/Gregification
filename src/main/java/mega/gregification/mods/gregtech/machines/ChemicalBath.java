...
public class ChemicalBath {
    ...
    public static void addRecipe(IItemStack[] output, IIngredient input, ILiquidStack fluidInput, int[] chances, int durationTicks, int euPerTick) {
        if (output.length == 0) {
            ...
            MineTweakerAPI.apply(new AddMultipleRecipeAction<GT_Recipe>("Adding Chemical Bath recipe for " + input, input, fluidInput, output[0],
                ...
                protected GT_Recipe applySingleRecipe(ArgIterator i) {
                    return RA.addChemicalBathRecipeRemovable(i.nextItem(), i.nextFluid(), i.nextItem(), i.nextItem(), i.nextItem(), i.nextIntArr(), i.nextInt(), i.nextInt());
                }

                @Override
                protected void undoSingleRecipe(GT_Recipe recipe) {
                    RA.removeChemicalBathRecipe(recipe);
                }
            });
        }
    }
    ...
    public static void addRecipe(IItemStack[] inputArray ,ILiquidStack[] inputFluidArray , IIngredient[] outputArray, ILiquidStack[] outputFluidArray,int[] chances, int durationTicks, int euPerTick) {
        if ((inputArray.length == 0 && inputFluidArray.length == 0) || (outputArray.length == 0 && outputFluidArray.length == 0)) {
            ...
            MineTweakerAPI.apply(new AddGTDirectRecipeAction(GT_Recipe.GT_Recipe_Map.sChemicalBathRecipes, "Adding Chemical Bath recipe for " + Arrays.toString(outputArray) + " : " + Arrays.toString(outputFluidArray),
